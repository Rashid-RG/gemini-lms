...
export const GenerateNotes=inngest.createFunction(
    ...
    async({event,step})=>{
        try {
            ...
            // Generate Notes for Each Chapter with AI - in separate steps to prevent Vercel serverless timeouts
            for (let index = 0; index < Chapters.length; index++) {
                ...
                await step.run(`Generate Chapter Notes - Chapter ${index + 1}`, async () => {
                    ...
                    for (let attempt = 0; attempt < 3 && !success; attempt++) {
                        try {
                            // Stagger requests - wait between attempts if needed
                            if (attempt > 0) {
...
- Max 1200 words, clean semantic HTML only, no CSS or style attributes`;
                    ...
                    // Check if notes already exist for this chapter to avoid duplicates during retries
    ...
    async ({ event, step }) => {
        try {
            ...
            const videoResult = await step.run('Fetch YouTube videos', async () => {
                ...
                // Search for videos for each chapter sequentially to avoid rate limits
