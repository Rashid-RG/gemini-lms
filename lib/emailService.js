...
class EmailService {
   ...
   */
  async sendLowCreditsWarning(email, firstName, remainingCredits) {
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          ...
          .amount { font-size: 26px; font-weight: 800; color: #f59e0b; letter-spacing: -0.5px; }
          .button { 
            display: inline-block; 
            padding: 14px 28px; 
            background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); 
      ...
      <body>
        <div class="container">
          <div class="header">
            <div style="font-size: 22px; font-weight: 700; letter-spacing: -0.5px; color: #ffffff; margin-bottom: 12px;">
              <span style="color: #cbd5e1; font-weight: 850;">Gemini</span><span style="color: #ffffff; font-weight: 300;"> LMS</span>✨
            </div>
            <h1>Credits Running Low!</h1>
            <p>Don't interrupt your learning journey</p>
          </div>
          <div class="content">
            <p style="font-size: 16px; margin: 0 0 20px 0;">Hi <strong>${firstName}</strong>,</p>
            <p style="font-size: 14px; margin: 0 0 20px 0;">We noticed that your AI course generation credits are running low. You have:</p>
            
            <div class="section">
              <h3>Available Balance</h3>
              <p style="margin: 6px 0; font-size: 14px;">Remaining Credits: <span class="amount">${remainingCredits}</span></p>
              <p style="margin: 10px 0 0 0; font-size: 13px; color: #b45309;">Each new course layout and study notes generation request consumes 1 credit.</p>
            </div>

            <p style="font-size: 14px; margin: 20px 0 20px 0;">To continue generating comprehensive customized courses, flashcards, and quizzes, you can purchase a credit pack or upgrade to our Premium Subscription for unlimited generation.</p>

            <div style="text-align: center; margin: 25px 0;">
              <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://geminilms.com'}/dashboard/upgrade" class="button">Refill Credits / Upgrade</a>
            </div>
    ...
    return this.transporter.sendMail({
      from: `Gemini LMS <${this.fromEmail}>`,
      to: email,
      subject: 'Attention: Your Course Generation Credits are Low!',
      html: htmlContent
    })
  }

  /**
   * Send subscription expiring soon warning email
   */
  async sendSubscriptionExpiringSoon(email, firstName, expirationDate) {
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          ...
          .header { 
            background: linear-gradient(135deg, #311042 0%, #1e1b4b 50%, #0f172a 100%); 
            color: white; 
            padding: 35px 24px; 
            text-align: center; 
          }
          ...
          .amount { font-size: 22px; font-weight: 800; color: #db2777; letter-spacing: -0.5px; }
          .button { 
            display: inline-block; 
            padding: 14px 28px; 
      ...
      <body>
        <div class="container">
          <div class="header">
            <div style="font-size: 22px; font-weight: 700; letter-spacing: -0.5px; color: #ffffff; margin-bottom: 12px;">
              <span style="color: #fbcfe8; font-weight: 850;">Gemini</span><span style="color: #ffffff; font-weight: 300;"> LMS</span>✨
            </div>
            <h1>Subscription Expiring Soon!</h1>
            <p>Keep your unlimited course creation active</p>
          </div>
          <div class="content">
            <p style="font-size: 16px; margin: 0 0 20px 0;">Hi <strong>${firstName}</strong>,</p>
            <p style="font-size: 14px; margin: 0 0 20px 0;">Your premium subscription for unlimited course generation is expiring soon:</p>
            
            <div class="section">
              <h3>Expiry Details</h3>
              <p style="margin: 6px 0; font-size: 14px;">Expiration Date: <span class="amount">${new Date(expirationDate).toLocaleDateString()}</span></p>
              <p style="margin: 10px 0 0 0; font-size: 13px; color: #9d174d;">To prevent any service interruption and maintain your access to unlimited courses, please renew your subscription.</p>
            </div>

            <div style="text-align: center; margin: 25px 0;">
              <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://geminilms.com'}/dashboard/upgrade" class="button">Renew Subscription</a>
            </div>
