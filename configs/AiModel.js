 ...
 * Called on every AI request so that key rotation takes effect immediately.
 */
...
// ─── Chat history templates (reused on every fresh chat) ───────────────

const courseOutlineHistory = [
  {
    role: "user",
    parts: [
...
// ─── Factory functions: create a FRESH chat with current rotated key ───
// Each call picks up whichever API key the rotation manager currently points to.
// If key 1 fails and rotation switches to key 2, the next call here uses key 2.

/**
 * Wrapper that creates a sendMessage-compatible object.
 * Every .sendMessage() call creates a brand-new chat session with the
 * currently-active (rotated) API key, so key rotation actually works.
 */
function createRotatingChat(chatConfig) {
  return {
    async sendMessage(message) {
      const model = getModel();
      const chat = model.startChat(chatConfig);
      const result = await chat.sendMessage(message);
      // Record success after the call actually succeeds
      try { getApiKeyRotationManager().recordSuccess(); } catch (_) {}
      return result;
    }
  };
}

export const courseOutlineAIModel = createRotatingChat({
  generationConfig,
  history: courseOutlineHistory,
});

export const generateNotesAiModel = createRotatingChat({
  generationConfig: generationConfig2,
});

export const GenerateStudyTypeContentAiModel = createRotatingChat({
  generationConfig,
  history: flashcardHistory,
});

export const GenerateQuizAiModel = createRotatingChat({
  generationConfig,
  history: quizHistory,
});

export const AssignmentGradingAiModel = createRotatingChat({
  generationConfig: {
    temperature: 0.7,
...
export const GenerateAssignmentsAiModel = createRotatingChat({
  generationConfig,
  history: assignmentsHistory,
});

export const GenerateMCQAiModel = createRotatingChat({
  generationConfig,
  history: mcqHistory,
});
